 ...
 *
 * AI quality pipeline for consultant documents before export (PPTX, PDF, DOCX).
 * Extracted from CDI generate.js — same prompts, same models, same fail-open behaviour.
 *
 ...
 *   }
 *   // goldExamples: pre-formatted string from your own DB query, or null
 ...
 * The caller is responsible for fetching gold standard examples from their DB.
 *
...
export interface OutputQuality {
  ...
  compressForPptx(content: string, goldExamples?: string | null): Promise<string>;
}
...
export function createOutputQuality(opts: OutputQualityOptions = {}): OutputQuality {
  ...
  async function reviewContent(content: string): Promise<ReviewResult> {
    const prompt = `You are a Deloitte-standard quality reviewer for LNS Research analyst deliverables.
...
- A conversational AI message (starts with "Let me...", "I'll...", "Sure...", "Ryan,", "I searched...", "The search...", "I found...")
...
2. LNS VOICE — Are specific frameworks named, data points cited, named companies used?
3. EXECUTIVE IMPACT — Would a COO or VP Manufacturing find this credible and actionable?
...
Write the COMPLETE revised version. Keep all substantive content. Fix only the identified issues — full prose for every section, no outlines, no AI preamble, no placeholder labels. Use markdown headings (##, ###) for structure. Reference the company's actual data and LNS frameworks from your context.`;
  ...
  async function compressForPptx(
    content: string,
    goldExamples: string | null = null,
  ): Promise<string> {
    const sections = content.split(/(?=^## )/m);
    const needsCompression = sections.some(s => countBodyChars(s) > PPTX_SECTION_CHAR_LIMIT);
    if (!needsCompression) return content;

    const goldSection = goldExamples
      ? `\nGOLD STANDARD REFERENCE — These are analyst-nominated deliverables. Study the section density, bullet conciseness, and prose style. Your output should match this quality level:\n\n${goldExamples}\n\n---\n\n`
      : '';
