/**
 * @lns-skills/market-data
 *
 * Stock/crypto price fetching with disk cache and rate limiting.
 * Uses free public APIs (Yahoo Finance via unofficial endpoints).
 *
 * Consumers: genome (event study), MI (backtesting), people (conviction)
 *
 * NOTE: For comprehensive stock analysis (scoring, signals, analyst ratings),
 * use @lns-skills/stock-analysis instead. This package is just price data.
 */
...
export async function fetchPrices(
  ...
  try {
    const r = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; LNS-MarketData/1.0)",
      },
