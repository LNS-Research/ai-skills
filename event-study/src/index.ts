...
const WINDOWS = [1, 5, 10, 30, 90, 180];
...
export async function runEventStudy(
  ...
  const tickerIndex = new Map(tickerData.prices.map((p, i) => [p.date, i]));
  const benchIndex = new Map(benchData.prices.map((p, i) => [p.date, i]));

  // Find the event date index (or nearest trading day after)
  let eventIdx = tickerIndex.get(eventDate);
  if (eventIdx === undefined) {
    // Find next trading day
    const sorted = tickerData.prices.map(p => p.date).sort();
    const nextDay = sorted.find(d => d >= eventDate);
    if (!nextDay) return null;
    eventIdx = tickerIndex.get(nextDay)!;
  }
